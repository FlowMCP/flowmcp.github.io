...
- Verfuegbare Schemas fuer ein Projekt auflisten willst (`flowmcp list`)
...
Durchsuche und rufe Schemas direkt aus dem CLI auf — jedes Tool aus den konfigurierten `schemaFolders[]` ist sofort aufrufbar, ohne Aktivierungsschritt:

```bash
flowmcp search ethereum
# Zeigt passende Schemas mit Beschreibungen und Parametern

flowmcp call get_contract_abi_etherscan '{"address": "0x..."}'
# Ruft das Tool direkt mit JSON-Parametern auf
```
