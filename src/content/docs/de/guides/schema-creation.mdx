...
`flowmcp grading deterministic` verwenden, um gegen die echte API zu verifizieren (HTTP 200 + nicht-leere Daten). Schema-Validierung allein kann API-seitige Probleme nicht erkennen.
:::
...
- Das `version: '4.1.0'`-Feld weglassen (erforderlich fuer alle Schemas)
:::
