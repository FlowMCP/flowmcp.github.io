...
1. Pick the data sources: `flowmcp search <topic>` then `flowmcp call <tool> '{...}'` (e.g. one crypto + one mobility schema) — every tool in the configured `schemaFolders[]` is immediately callable, no activation step
