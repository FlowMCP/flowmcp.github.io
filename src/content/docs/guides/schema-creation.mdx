...
This guide walks you through creating FlowMCP v4.1.0 schemas. A schema is a `.mjs` file that describes how to interact with a REST API -- what endpoints exist, what parameters they accept, and how responses should be transformed.
...
6. **Test with CLI**
   ...
   ```bash
   flowmcp grading deterministic <namespace>/<schema-name>
   flowmcp grading deterministic <namespace>/tool/getData
   ```
...
Use `flowmcp grading deterministic` to verify against the real API (HTTP 200 + non-empty data). Schema validation alone cannot catch API-side issues.
:::
...
- Omitting the `version: '4.1.0'` field (required for all schemas)
:::
