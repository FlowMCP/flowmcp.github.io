...
---
{/* PAGEFIND-META-START */}
<span style="display:none" data-pagefind-meta="section">Quickstart</span>
{/* PAGEFIND-META-END */}
...
<Aside type="tip" title="Hackathon participants">
    Building at a hackathon? Run `claude /flowmcp-hackathon-onboarding` for a pre-filled skill that scaffolds your MCP server in under 60 seconds.
    [Read the full Hackathon Kit →](/guides/hackathon-kit/)
</Aside>

## Install the CLI

The FlowMCP CLI is the fastest path to a working setup. It lets you search the 728-tool catalog, add schemas, and call APIs without writing code.
...
`flowmcp list` is empty on a fresh install — that is expected. Continue below to add your first schema.

## Add Your First Schema (CLI mode)
...
Three commands and you have a live API call. The CLI is the recommended path for hackathons, quick prototypes, and exploring the catalog.
...
Prefer to embed FlowMCP in your own Node.js code? Install the core library instead of the CLI:
...
:::note[CLI Reference]
Search, activate, and call {schemaCount} pre-built schemas from the command line. See [CLI Usage](/reference/cli/).
:::
