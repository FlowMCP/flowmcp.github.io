...
Prompts are explanatory texts scoped to a namespace. They teach AI agents how a provider's tools work together -- pagination patterns, error codes, rate limits, combining endpoints. Prompts **explain**, they don't **instruct** (that's what [Skills](/concepts/schemas-skills/) do).
...
See [Prompt Architecture](/specification/prompt-architecture/) for the full two-tier system (Provider-Prompts vs Agent-Prompts).
