...
:::note
Group-level skills (`.md` files in `.flowmcp/skills/`) are different from schema-level skills (`.mjs` files alongside the schema). Group-level skills are project-specific workflows. Schema-level skills are part of the schema's MCP Prompts. See [Skills](/specification/skills/) for schema-level skill documentation.
:::
