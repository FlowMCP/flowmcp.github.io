...
:::note
This page covers the schema format from the [formal specification](https://github.com/FlowMCP/flowmcp-spec). See [Parameters](/specification/parameters/) for parameter details and [Security Model](/specification/security/) for security constraints.
:::
...
| `sharedLists` | `object[]` | `[]` | Shared list references. See [Shared Lists](/specification/shared-lists/). |
| `resources` | `object` | `{}` | SQLite-based read-only data resources. See [Resources](/specification/resources/). |
| `skills` | `array` | `[]` | AI agent skill references. See [Skills](/specification/skills/). |
...
| `output` | `object` | No | Output schema. See [Output Schema](/specification/output-schema/). |
| `preload` | `object` | No | Cache configuration. See [Preload](/specification/preload/). |
