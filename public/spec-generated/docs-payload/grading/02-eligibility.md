---
title: "Eligibility"
description: "This chapter defines **what is allowed to be part of a gradable schema**. Eligibility is the upstream gate: an endpoint that is not eligible MUST NOT appear in a schema, and the chapter sets out the..."
...
source_url: "https://github.com/FlowMCP/flowmcp-spec/blob/659863f/grading/3.0.0/02-eligibility.md"
...
A schema that contains write/update/delete tools without an explicit, documented justification will be flagged under [Exclusion Criteria](#exclusion-criteria) below.
...
| Both schemas — the free schema and the API-key schema — are individually maximalist under [`01-default-journey.md`](/grading/default-journey/) | MUST |
...
The eligibility rules of [Exclusion Criteria](#exclusion-criteria)–[Target Audience for Data Sources](#target-audience-for-data-sources) are verified during the grading areas defined in [`04-phases-single.md`](/grading/phases-single/):

- [Exclusion Criteria](#exclusion-criteria) is enforced before the `single-test` area runs — the endpoint list MUST already be eligibility-classified.
- [Access Classes](#access-classes) and [Schema Splitting](#schema-splitting) are reflected in the `single-test` and `tools-aggregate-schema` areas.
